...
def compute_forward_returns(factor,
    ...
    freq : str
        The frequency of the data in the input `prices`
    ...
    """
    factor.index.levels[0].freq = factor.index.levels[0].inferred_freq
    ...
    for period in sorted(periods):
        ...
        _, period_length, period_type = re.split('(\d+)', freq)
        label = '{0}{1}'.format(period * int(period_length), period_type)
...
def get_clean_factor_and_forward_returns(factor,
    ...
    prices : pd.DataFrame
        ...
        ::
            ...
            ----------------------------------------------------
    freq : str, optional
        The frequency of the data. Must be '1D', '1M', '1Y', '1W', or else alias of time delta
    groupby : pd.Series - MultiIndex or dict
        Either A MultiIndex Series indexed by date and asset,
        containing the period wise group codes for each asset, or
        a dict of asset to group mappings. If a dict is passed,
...
def get_forward_returns_columns(columns, require_exact_day_multiple=True):
    """
    ...
    if require_exact_day_multiple:
        pattern = re.compile(r"^(\d+([DHTS]))$", re.IGNORECASE)
...
def timedelta_strings_to_integers(sequence):
    ...
    """
    return [int(re.split('(\d+)', x)[1]) for x in sequence]
