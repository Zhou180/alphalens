...
def create_returns_tear_sheet(
    ...
    # Compute cumulative returns from base freq simple returns, if basefreq("D"、"4H"...)
    # returns are provided.
    basefreq = factor_data.index.levels[0].freqstr
    if basefreq is 'D':
        basefreq = '1D'
    if basefreq in factor_returns:
        title = (
            "Factor Weighted "
            + ("Group Neutral " if group_neutral else "")
            + ("Long/Short " if long_short else "")
            + f"Portfolio Cumulative Return ({basefreq} Period)"
        )

        plotting.plot_cumulative_returns(
            factor_returns[basefreq], period=basefreq, title=title, ax=gf.next_row()
        )

        plotting.plot_cumulative_returns_by_quantile(
            mean_quant_ret_bydate[basefreq], period=basefreq, ax=gf.next_row()
        )
...
def create_turnover_tear_sheet(factor_data, turnover_periods=None):
    ...
    factor_data : pd.DataFrame - MultiIndex
        ...
        (optionally) the group the asset belongs to.
        - See full explanation in utils.get_clean_factor_and_forward_returns
    turnover_periods : sequence[string]
